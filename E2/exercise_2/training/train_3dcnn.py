    ...
    for epoch in range(config['max_epochs']):
        for i, batch in enumerate(trainloader):
            ...
            for output_idx in range(prediction.shape[1]):
                cur_loss = loss_criterion(prediction[:, output_idx, :], batch['label'])
                loss_total = loss_total + cur_loss  # TODO: Loss due to prediction[:, output_idx, :] (output_idx=0 for global prediction, 1-8 local)
            ...
            if iteration % config['validate_every_n'] == (config['validate_every_n'] - 1):
                ...
                for batch_val in valloader:
                    ...
                    for j in range(predicted_label.shape[1]):
                        cur_loss = loss_criterion(prediction[:, j, :], batch_val['label'])
                        loss_total_val += cur_loss
